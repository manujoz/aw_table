...
class AwTable extends PolymerElement {
        ...
        return html`
            <style>
                ...
                [mark-row] tbody tr:nth-of-type(2n) {
                    color: var(--aw-table-row-mark-color, var(--aw-table-cell-color, #333));
                    background-color: var(--aw-table-row-mark-background-color, #f6f6f6);
                }
                [mark-column] tbody td:nth-of-type(2n) {
                    color: var(--aw-table-column-mark-color, var(--aw-table-cell-color, #333));
                    background-color: var(--aw-table-column-mark-background-color, #f6f6f6);
                }
                ...
                [row-effect] tbody tr:hover {
                    background-color: var(--aw-table-cell-background-color-hover, #e6e6e6);
                }
                [sortable] {
                    cursor: pointer;
    ...
    getColumnsSizes() {
        if (this.head.element) {
            this.head.cells.forEach((cell, c) => {
                this.columnsWidht[c] = cell.hasAttribute("width") ? cell.getAttribute("width") : null;
            });
        } else {
    ...
    innerConstruct() {
        this.head.cells.forEach((cell, c) => {
            ...
            if (cell.hasAttribute("min-width")) {
                cell.style.minWidth = cell.getAttribute("min-width");
            }

            cell.setAttribute("nowrap", "");
            cell.setAttribute("title", cell.innerHTML);
            cell.innerHTML = `<div style="${this.columnsWidht[c] ? `width: ${this.columnsWidht[c]}` : ``}">${cell.innerHTML}</div>`;
        });
    ...
    parseBody() {
        ...
        rows.forEach((row) => {
            ...
            if (this.body.rows[i].cells.length === 0) {
                console.error("[aw-table]: <tbody><tr> element must contains one cell almost");
                return false;
            }
        });
    ...
    parseFooter() {
        ...
        if (this.footer.cells.length === 0) {
            console.error("[aw-table]: <tfoot> element must contains one cell almost");
            return false;
        }
    ...
    parseHead() {
        ...
        if (this.head.cells.length === 0) {
            console.error("[aw-table]: <thead> element must contains one cell almost");
            return false;
        }
    ...
    sortClick(cell) {
        this.head.cells.forEach((ce) => {
            if (ce.hasAttribute("sortable") && ce !== cell) {
                ...
                ce.querySelector(".aw-sort-icon").setAttribute("icon", "arrow-downward");
            }
        });
        ...
        if (dir === "asc") {
            ...
            cell.setAttribute("dir", "asc");
            cell.querySelector(".aw-sort-icon").setAttribute("icon", "arrow-downward");
        }
